...
minExplainedVarianceStim = 0.016;
minPVal = 0.05;
maxLambda = 0.01;
...
for subj = 1:length(subjects)
    ...
    for dt = 1:length(dates)
        ...
        for iUnit = 1:length(validRF)
            ...
            tiledlayout(2, numEyeBins, "TileSpacing", "tight")
            rf = squeeze(RFs(iUnit,:,:,mxTime,:,:));
            mx = max(abs(rf(:)));
            for sub = 1:2
                for pos = 1:numEyeBins
                    nexttile
                    imagesc(...
            ...
            sgtitle(sprintf('Neuron %d: %s (w/o shift: EV: %.3f, p: %.3f, lam: %.3f)', ...
                cellID, RFtypes{mxSub}, rfData.explVars(cellID), ...
        ...
        rfMapsShifted = NaN([size(rfData.maps), numEyeBins]);
        ...
        writeNPY(eyePosPerBin, fullfile(folderRes, 'eyeDistribution.xPos.npy'))
        ...
        % stimulus edges
        rfGaussShift(rfGaussShift < noiseData.edges(1) | ...
            rfGaussShift > noiseData.edges(2)) = NaN;
        n = all(~isnan(rfGaussShift), 2);

        figure('Position', [680 460 990 420])
        tiledlayout(1, 2)
        nexttile
        plot(eyePosPerBin, rfGaussShift, 'Color', [1 1 1].*0.5)
        hold on
        if sum(n) > 0
            plot(eyePosPerBin, median(rfGaussShift(n,:), 1, 'omitnan'), 'k', 'LineWidth', 2)
        end
        set(gca, 'box', 'off')
        axis tight
        xlabel('Pupil position (pos-median)/(25-75 prctile range)')
        ylabel('Horiz. RF centre (vis deg)')

        if sum(n) > 0
            nexttile
            posNorm = rfGaussShift - median(rfGaussShift, 2, 'omitnan');
            mdl = fitlm(reshape(repmat(eyePosPerBin, sum(n), 1), [], 1), ...
                reshape(posNorm(n,:), [], 1), "RobustOpts", "on");
            coeffs = mdl.Coefficients.Estimate;

            plot(eyePosPerBin, posNorm(n,:), 'Color', [1 1 1] .* 0.5)
            hold on
            plot(eyePosPerBin, median(posNorm(n,:), 1, 'omitnan'), 'k', 'LineWidth', 2)
